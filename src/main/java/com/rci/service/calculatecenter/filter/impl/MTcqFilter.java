...
public class MTcqFilter extends AbstractPaymodeFilter {
	...
	protected void doExtractOrderInfo(ParameterValue value) {
		...
		if(onlineAmount.compareTo(payAmount) > 0){
			...
			logger.warn("---["+value.getPayNo()+"-美团超券支付异常]-最大可在线支付金额是"+payAmount+",实际在线支付金额是"+onlineAmount);
			value.joinWarningInfo("["+value.getPayNo()+"-美团超券支付异常]-最大可在线支付金额是"+payAmount+",实际在线支付金额是"+onlineAmount);
		}
		...
		try {
			...
			if(order.getCheckoutTime().after(DateUtil.getTimeOfDay(orderDate, 22, 0, 0, 0))){  
				//过了22:00到店付没有优惠
				value.addPayInfo(PaymodeCode.MTSUPER, onlineAmount);
				return;
			}
			
			if(onlineAmount.compareTo(originAmount) == 0){//全部在美团超券平台支付
				payAmount = originAmount.subtract(nodiscountAmount);
			}else{ //不可打折金额使用另外支付方式（如现金）等支付
				payAmount = onlineAmount;
			}
			BigDecimal[] result = calculator.doCalculateAmountForOnlinePay(payAmount,orderDate,Vendor.MTSUPER);
			value.addPayInfo(PaymodeCode.MTSUPER, result[0]);
