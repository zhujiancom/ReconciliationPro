...
public class InventoryWarningLineCellRender extends DefaultCellEditor implements FocusListener{
	/**
	 * 
	 */
	private static final long serialVersionUID = -2195188539660680534L;
	
	private static final Log logger = LogFactory.getLog(InventoryWarningLineCellRender.class);
	...
	public InventoryWarningLineCellRender(JTextField field) {
		super(field);
		this.field = field;
	...
	public void focusLost(FocusEvent event) {
		...
		if(StringUtils.hasText(field.getText())){
			try{
				metaservice.warningLineSetting(getIno(),new BigDecimal(field.getText()));
			}catch(Exception ex){
